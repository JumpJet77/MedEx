...
import ua.edu.viti.medex.main.dao.interfaces.IPhysicalDataDAO;
...
@Service
public class PhysicalDataDAOImpl implements IPhysicalDataDAO {
	...
	@Override
	public List<PhysicalData> getAllPhysicalDataByPatient(Long id) {
		CriteriaBuilder cb = mainSessionFactory.getCriteriaBuilder();
		CriteriaQuery<PhysicalData> cq = cb.createQuery(PhysicalData.class);
		Root<PhysicalData> root = cq.from(PhysicalData.class);
		cq.select(root).where(cb.equal(root.get("patient"), id));
		Query query = mainSessionFactory.getCurrentSession().createQuery(cq);
		List<PhysicalData> searchedRolesList = query.getResultList();
	...
	@Override
	public List<PhysicalData> getAllPhysicalDataByPatient(String email) {
		CriteriaBuilder cb = mainSessionFactory.getCriteriaBuilder();
		CriteriaQuery<PhysicalData> cq = cb.createQuery(PhysicalData.class);
		Root<PhysicalData> root = cq.from(PhysicalData.class);
	...
	@Override
	public Long addPhysicalData(PhysicalData physicalData) {
		mainSessionFactory.getCurrentSession().persist(physicalData);
		return physicalData.getId();
	}
}
